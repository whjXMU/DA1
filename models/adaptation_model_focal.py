...
class CustomModel():
    def __init__(self, opt, logger,gpu, isTrain=True):
        self.opt = opt
    ...
    def seg(self, source_x, source_label, target_x, target_imageS=None, target_params=None, target_lp=None, 
    ...
    def step(self, source_x, source_label, target_x, target_imageS=None, target_params=None, target_lp=None, 
        ...
        if self.opt.S_pseudo > 0:
            ...
            cluster_argS = self.label_strong_T(cluster_arg.clone().clone().float(), target_params, padding=250, scale=4).to(torch.int64)
        ...
        pred_trg_entropy = self.prob_2_entropy(F.softmax(target_out['out'].detach()))
        loss_CTS = 0
        for i in range(batch):
            with torch.no_grad():
                out_label= target_out['out'].cpu().data[i].numpy()
                out_label = out_label.transpose(1,2,0)
                out_label = np.argmax(out_label, axis=2)
                out_label=np.tile(out_label,(2,1,1))
                n= out_label.sum()/2
                if n<=100:
                    entropy_mean = 1
                else:
                    pred_trg_entropy_all = (pred_trg_entropy[i].cpu()*out_label).sum().item()
                    entropy_mean = (pred_trg_entropy_all/n)
                    # if entropy_mean < 0.8:
                    #     entropy_mean = entropy_mean + 0.2
            loss_CTS_ = cross_entropy2d(input=target_out['out'][i].unsqueeze(0), target=threshold_arg[i].unsqueeze(0).reshape([1, w, h]))
            loss_CTS_ = loss_CTS_ * (entropy_mean ** 2)
            # print("bili:{},loss_bili:{}".format((entropy_mean),loss_CTS))
            loss_CTS=loss_CTS+loss_CTS_
        
        # print("loss_CTS_all:{}".format(loss_CTS_all))
