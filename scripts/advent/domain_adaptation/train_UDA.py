...
def eval_single(cfg, model,device, val_loader,logger):
    ...
    print(f'IoU = \t{inters_over_union_classes[1]}')
    
    return inters_over_union_classes[1]

        
def train_advent(model, trainloader, targetloader, val_loader,cfg,logger):
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP + 1)):
        ...
        # print("shape of pred_src_main, labels!!!!!!!!!!",pred_src_main.shape, labels.shape,np.unique(labels))
        ...
        # print("type:{}".format(pred_trg_main.shape))
        # return 1
        ...
        if i_iter % 500 == 0 and i_iter != 0:
            iou=eval_single(cfg,model,device, val_loader,logger)
            if best_iou < iou:
                best_iou = iou
                torch.save(model.state_dict(), "/home/hjw/advent/advent/logs/2/" + 'model_best.pth')
                torch.save(d_aux.state_dict(), "/home/hjw/advent/advent/logs/2/" + 'model_best_D_aux.pth')
                torch.save(d_main.state_dict(), "/home/hjw/advent/advent/logs/2/" + 'model_best_D_main.pth')
                logger.info(f'the best IoU util now is {best_iou}')
            model.train()
        ...
        if i_iter % 500 == 0 and i_iter != 0:
            ...
            torch.save(model.state_dict(), "/home/hjw/advent/advent/logs/2/" + 'model_current.pth')
            torch.save(d_aux.state_dict(), "/home/hjw/advent/advent/logs/2/" +  'model_current_D_aux.pth')
            torch.save(d_main.state_dict(),"/home/hjw/advent/advent/logs/2/" + 'model_current_D_main.pth')
...
def train_domain_adaptation(model, trainloader, targetloader,val_loader,cfg,logger):
    if cfg.TRAIN.DA_METHOD == 'MinEnt':
        train_minent(model, trainloader, targetloader, cfg)
    elif cfg.TRAIN.DA_METHOD == 'AdvEnt':
        train_advent(model, trainloader, targetloader, val_loader,cfg,logger)
    else:
