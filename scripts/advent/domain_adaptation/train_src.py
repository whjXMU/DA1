...
def train_advent(model, trainloader, targetloader, cfg):
    ...
    if viz_tensorboard:
        writer = SummaryWriter(log_dir=cfg.TRAIN.TENSORBOARD_LOGDIR +'/src')
    ...
    for i_iter in tqdm(range(cfg.TRAIN.EARLY_STOP + 1)):
        ...
        if i_iter % cfg.TRAIN.SAVE_PRED_EVERY == 0 and i_iter != 0:
            ...
            snapshot_dir = Path(cfg.TRAIN.SNAPSHOT_DIR) /'src'
...
def train_src_only(model, trainloader, targetloader, cfg):
