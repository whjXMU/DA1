...
def KL(images,model1,model2):
    ...
    loss_kl = 0.1*F.kl_div(student, teacher, reduction='none')
    # print(loss_1.sum())
        ...
        for data_i_E, data_i_H in zip(datasets.target_train_loader_E,datasets.target_train_loader_H):
            ...
            if (i + 1) % opt.print_interval == 0:
                if opt.stage == 'warm_up':
                    ...
                    fmt_str = "Epochs [{:d}/{:d}] Iter [{:d}/{:d}]  Time/Image: {:.4f} \n loss1: {:.4f}  loss_CTS1: {:.4f}  loss_consist1: {:.4f} loss_pixel1: {:.4f}\n loss2: {:.4f}  loss_CTS2: {:.4f}  loss_consist2: {:.4f} loss_pixel2: {:.4f}\nloss3: {:.4f}  loss_CTS3: {:.4f}  loss_consist3: {:.4f} loss_pixel3: {:.4f}\n loss4: {:.4f}  loss_CTS4: {:.4f}  loss_consist4: {:.4f} loss_pixel4: {:.4f}"
                    print_str = fmt_str.format(epoch+1, opt.epochs, i + 1, opt.train_iters, time_meter.avg / opt.bs,loss1, loss_CTS1, loss_consist1,loss_pixel1,loss2, loss_CTS2, loss_consist2,loss_pixel2,loss3, loss_CTS3, loss_consist3,loss_pixel3,loss4, loss_CTS4, loss_consist4,loss_pixel4 )
                else:
                    print('stage 2')
