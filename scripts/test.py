...
from generate_pseudo_label import label2rgb
...
def test(opt, logger):
    ...
    print('target_easy___________')
    validation(model, logger, datasets.target_train_E_test_loader, device, running_metrics_val)
    print('target_hard___________')
    validation(model, logger, datasets.target_train_H_test_loader, device, running_metrics_val)
    print('val___________')
    validation(model, logger, datasets.target_valid_loader, device, running_metrics_val)
    print('test___________')
    validation(model, logger, datasets.target_test_loader, device, running_metrics_val)
...
def validation(model, logger, datasets, device, running_metrics_val):
    ...
    with torch.no_grad():
        ...
        validate(datasets, device, model, running_metrics_val)
...
def validate(valid_loader, device, model, running_metrics_val):
    ...
    for data_i in tqdm(valid_loader):
        ...
        outputs_rgb = F.interpolate(sm(outs['out']), size=images_val.size()[2:], mode='bilinear', align_corners=True)
        pred = outputs.data.max(1)[1].cpu().numpy()
        pred_rgb = outputs_rgb.data.max(1, keepdim=True)[1]
        for k in range(labels_val.shape[0]):
            name = os.path.basename(filename[k])
            Image.fromarray(pred_rgb[k,0].cpu().numpy().astype(np.uint8)).save(os.path.join("/home/hjw/proda-mine/Dataset/predict/", name))
        pred_rgb = label2rgb(valid_loader.dataset.decode_segmap, pred_rgb).float() * 255
        for k in range(labels_val.shape[0]):
            name = os.path.basename(filename[k])
            Image.fromarray(pred_rgb[k].permute(1,2,0).cpu().numpy().astype(np.uint8)).save(os.path.join("/home/hjw/proda-mine/Dataset/predict_color/", name[:-4] + '_color.png'))
