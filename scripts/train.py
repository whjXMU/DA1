    ...
    for epoch in range(start_epoch, 20):
        for data_i in datasets.target_train_loader:
            ...
            if (i + 1) % opt.val_interval == 0:
                ...
                # validation1(model1, logger, datasets, device, running_metrics_val1, iters = model.iter, opt=opt)
...
def validation1(model, logger, datasets, device, running_metrics_val, iters, opt=None):
    iters = iters
    _k = -1
    for v in model.optimizers:
        _k += 1
    ...
    state['iter'] = iters + 1
    state['best_iou'] = score["Mean IoU : \t"]
    save_path = os.path.join(opt.logdir,"from_{}_to_{}_on_{}_current_model1.pkl".format(opt.src_dataset, opt.tgt_dataset, opt.model_name))
    torch.save(state, save_path)
    ...
    if score["Mean IoU : \t"] >= model.best_iou:
        ...
        state['iter'] = iters + 1
        state['best_iou'] = model.best_iou
        save_path = os.path.join(opt.logdir,"from_{}_to_{}_on_{}_best_model1.pkl".format(opt.src_dataset, opt.tgt_dataset, opt.model_name))
        torch.save(state, save_path)
        return score["Mean IoU : \t"]
