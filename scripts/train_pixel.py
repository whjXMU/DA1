...
import adaptation_modelv2_pixel as adaptation_modelv2
...
def train(opt, logger):
    ...
    for epoch in range(start_epoch, opt.epochs):
        for data_i_E, data_i_H in zip(datasets.target_train_loader_E,datasets.target_train_loader_H):
            ...
            target_label_E = data_i_E['label'].to(device)
            ...
            target_label_H = data_i_H['label'].to(device)
            ...
            if opt.stage == 'stage1':
                ...
                loss_pixel1, _ = model1.association( images_E, labels_E, target_image_E, target_label_E)
                ...
                loss_pixel2, _ = model2.association( images_E, labels_E, target_image_H, target_label_H)

                loss3, loss_CTS3, loss_consist3 = model3.step(images_H, labels_H, target_image_E, target_imageS_E, target_params_E, target_lp_E,
                                        target_lpsoft_E, target_image_full_E, target_weak_params_E)
                loss_pixel3, _ = model3.association( images_H, labels_H, target_image_E, target_label_E)

                loss4, loss_CTS4, loss_consist4 = model4.step(images_H, labels_H, target_image_H, target_imageS_H, target_params_H, target_lp_H,
                                        target_lpsoft_H, target_image_full_H, target_weak_params_H)
                loss_pixel4, _ = model4.association( images_H, labels_H, target_image_H, target_label_H)
            ...
            if (i + 1) % opt.print_interval == 0:
                if opt.stage == 'warm_up':
                    ...
                    fmt_str = "Epochs [{:d}/{:d}] Iter [{:d}/{:d}]  Time/Image: {:.4f} \n loss1: {:.4f}  loss_CTS1: {:.4f}  loss_consist1: {:.4f} loss_pixel1: {:.4f} loss2: {:.4f}  loss_CTS2: {:.4f}  loss_consist: {:.4f} loss_pixel2: {:.4f}"
                    print_str = fmt_str.format(epoch+1, opt.epochs, i + 1, opt.train_iters, time_meter.avg / opt.bs,loss1, loss_CTS1, loss_consist1, loss_pixel1, loss2, loss_CTS2, loss_consist2, loss_pixel2)
                else:
                    print('stage 2')
...
def validation(model, logger, datasets, device, running_metrics_val, iters,num, opt=None):
    ...
    save_path = os.path.join(opt.logdir,"from_{}_to_{}_on_{}_current_model{}_pixel.pkl".format(opt.src_dataset, opt.tgt_dataset, opt.model_name, num))
    # save_path=os.path.join(opt.logdir,"S_all_T_easy_best_pixel.pkl")
    if class_iou[1]>= model.best_iou:
        ...
        # save_path = os.path.join(opt.logdir,"from_{}_to_{}_on_{}_best_model{}_pixel.pkl".format(opt.src_dataset, opt.tgt_dataset, opt.model_name,num))
        save_path = os.path.join(opt.logdir,"from_{}_to_{}_on_{}_best_model{}_pixel.pkl".format(opt.src_dataset, opt.tgt_dataset, opt.model_name, num))
