...
from  ..models.pixel_loss import AssociationLoss,SegCrossEntropyLoss
from utlis import to_cuda, get_world_size
from .. import pixel_utils as solver_utils
...
class CustomModel():
    def __init__(self, opt, logger,gpu, isTrain=True):
        ...
        #pixel_level-cycle
        self.CELoss = SegCrossEntropyLoss(
                    ignore_index=-1,
                    ds_weights=(1.0, 0.4))
    ...
    def step(self, source_x, source_label, target_x, target_imageS=None, target_params=None, target_lp=None, 
        ...
        return loss, loss_CTS.item(), loss_consist.item()
...
def association(self, data_S, gt_S, data_T, gt_T):
        ...
        if 10 > 0.0:
            lsr_thres = 10
            lsr_loss_S = solver_utils.LSR(F.log_softmax(preds_S, dim=1), dim=1, thres=10)
            lsr_loss_T = solver_utils.LSR(F.log_softmax(preds_T, dim=1), dim=1, thres=10)

            total_loss += 0.01 * lsr_loss_S
            total_loss += 0.01* lsr_loss_T
        ...
        ce_loss += (0.75 * get_world_size() * self.iter_size) * lov_loss
